...
vector<PII> parents[inf+1];
vector<int> parAtDepth[inf+1];
int par[inf+1], val[inf+1];
    ...
    // FastIO;
     ...
     freopen("out.txt", "w", stdout);
  	...
  	while (t--) {
  		...
  		par[0] = 0, val[0] = 1;
  		parAtDepth[0].push_back(0);
  		parents[0].push_back({1, 0});
  		for (int i=1; i<n; i++) {
  			int x, y;
  			scanf("%d %d", &x, &y);
  			par[i] = x;
  			val[i] = y;
  			parents[i].push_back({y, 0});
  			parAtDepth[i].push_back(i);
  		}
  		for (int i=1; i<n; i++) {
  			int currentParent = par[i], depth = 1;
  			parents[i].push_back({val[currentParent], depth});
  			parAtDepth[i].push_back(currentParent);
  			while (currentParent != 0) {
  				depth++;
  				currentParent = par[currentParent];
  				parents[i].push_back({val[currentParent], depth});
  				parAtDepth[i].push_back(currentParent);
  			}
  		...
  		while (q--) {
  			int k, v;
  			scanf("%d %d", &k, &v);
  			int left = 0, right = parents[k].size() -1, mid, ans;
  			auto pos = *(lower_bound(parents[k].begin(), parents[k].end(), MP(v, 0)));
  			printf("%d\n", parAtDepth[k][pos.second]);
  		}
  		clear();
  	}
