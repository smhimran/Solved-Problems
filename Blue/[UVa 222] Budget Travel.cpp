...
double dis[100], price[100];

double capacity, milage, cost;

int citys;

double solve(int i, double cap) {
  ...
  if (i>0) 
    path -= dis[i-1];
  cap -= path/milage;
	if (cap*2 <= capacity or cap*milage < abs(dis[i+1]-dis[i])) {
    double needed = capacity - cap;
    needed = round(needed * price[i]);
    ret = min(ret, 200 + needed + solve(i+1, capacity));
  }
  if (cap*milage >= abs(dis[i+1]-dis[i])) 
...
int main()
{
    // FastIO;
     ...
     freopen("out.txt", "w", stdout);
  	...
  	double dist;
    int ca=1;
  	while (scanf("%lf", &dist) != EOF) {
      if (dist == -1)
        break;
    	int i;
    	scanf("%lf %lf %lf %d", &capacity, &milage, &cost, &citys);
    	for (i=0; i<citys; i++) 
    		scanf("%lf %lf", &dis[i], &price[i]);

      dis[i] = dist;
      cost *= 100.00;
      cost += solve(0, capacity);
      cost /= 100.00;
      printf("Data Set #%d\n", ca++);
