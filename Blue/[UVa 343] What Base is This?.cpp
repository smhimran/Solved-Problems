...
int main()
{
	...
	while (cin>>a>>b) {
		...
		for (int i=0; i<len(a); i++) {
			if (a[i]>='0' and a[i]<='9')
				min_a = max(min_a, a[i]-'0');
			else 
				min_a = max(min_a, a[i] - 'A' + 10);
		}

		for (int i=0; i<len(b); i++) {
			if (b[i]>='0' and b[i]<='9')
				min_b = max(min_b, b[i]-'0');
			else 
				min_b = max(min_b, b[i] - 'A' + 10);
		}

		bool found = 0;
		int i, j;
		for (i=min_a + 1; i<=36; i++) {
			int val_a = getValue(a, i);
			for (j=min_b + 1; j<=36; j++) {
				int val_b = getValue(b, j);
				if (val_b == val_a) {
					found = 1;
					break;
				}
