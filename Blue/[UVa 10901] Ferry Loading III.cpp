    ...
    // FastIO;
     ...
     freopen("out.txt", "w", stdout);
  	...
  	while (ca--) {
  		...
  		bool left_now = 1;
  		int time = 0, curr = 0, i=0, j=0;
  		while (!left.empty() or !right.empty()) {

  			while (curr) {
  				curr--;
  				if (left_now and j<right_index.size()) {
  					ans[right_index[j]] = time;
  					j++;
  				}
  				else if (i<left_index.size()) {
  					ans[left_index[i]] = time;
  					i++;
  				}
  			}
  			...
  			if (left_now) {
  				while (!left.empty() and left.front() <= time and curr < n) {
  					curr++;
  					left.pop();
  				}
  			}
  			else {
  				while (!right.empty() and right.front() <= time and curr < n) {
  					curr++;
  					right.pop();
  				}
  			}
