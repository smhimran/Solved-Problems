...
void precal() {
	int i=0;
	while (i<prime.size() and prime[i]<=32000) {
		if ((prime[i+1] - prime[i] == prime[i+2] - prime[i+1]) and prime[i+1]<=32000 and prime[i+2]<=32000) {
			int diff = prime[i+1] - prime[i];

			v[ind].push_back(prime[i]);
			v[ind].push_back(prime[i+1]);
			v[ind].push_back(prime[i+2]);
			i += 2;

			while(prime[i+1]-prime[i]==diff){
                v[ind].push_back(prime[i+1]);
                i++;
            }
