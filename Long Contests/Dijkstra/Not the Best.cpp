...
void dijkstra(int source=1) {
    ...
    while (!pq.empty()) {
        ...
        for (auto i: v[u]) {
            int curr = d + i.second;
            if (curr < dis[i.first].first) {
                dis[i.first] = {curr, dis[i.first].first};
                pq.push(node(i.first, curr));
            }
            else if (curr > dis[i.first].first and curr < dis[i.first].second) {
                dis[i.first] = {dis[i.first].first, curr};
                pq.push(node(i.first, curr));
            }
        }
    }
...
int main()
{
    ...
    while (t--) {
        ...
        dijkstra();
        cout<<"Case "<<ca++<<": "<<dis[n].second<<endl;
    }
