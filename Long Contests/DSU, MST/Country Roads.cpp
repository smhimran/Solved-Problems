...
void dfs(int i, int max_cost) {
	for (auto u: v[i]) {
		int des = u.first, weight = u.second;
		if (!visited[des]) {
			visited[des] = 1;
			cost[des] = max(max_cost, weight);
			dfs(des, max(max_cost, weight));
		}
	}
}
    ...
    FastIO;
     ...
     freopen("out.txt", "w", stdout);
  	...
  	while (t--) {
  		...
  		for (int i=0; i<m; i++) {
  			int x = edges[i].x, y = edges[i].y, weight = edges[i].weight;
  			if (find(x) != find(y)) {
  				uni(x, y);
