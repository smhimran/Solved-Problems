     ...
     freopen("out.txt", "w", stdout);
  	...
  	while (t--) {
  		...
  		while (!q.empty()) {
  			...
  			visited[now] = 1;
  			if (now.first == x2 and now.second == y2) {
  				cost = level[now];
  				break;
  			}
  			q.pop();
  			LL next_x = (now.first + 2LL*(now.second)), next_y = now.second;
  			PLL east = {next_x, next_y};
  			if (!visited[east]) {
  				q.push(east);
  				level[east] = level[now] + 1;
  			}
  			next_x = (now.first - 2LL*(now.second)), next_y = now.second;
  			PLL west = {next_x, next_y};
  			if (!visited[west]) {
  				q.push(west);
  				level[west] = level[now] + 1;
  			}
  			next_x = now.first;
  			LL val = (now.second + 2LL * now.first);
  			if (val > 0)
  				next_y = val;
  			else {
  				next_x *= -1LL;
  			...
  			PLL north = {next_x, next_y};
  			if (!visited[north]) {
  				q.push(north);
  				level[north] = level[now] + 1;
  			}
  			next_x = now.first;
  			val = (now.second - 2LL * now.first);
  			if (val > 0)
  				next_y = val;
  			else {
  				next_x *= -1LL;
  			...
  			PLL south = {next_x, next_y};
  			if (!visited[south]) {
  				q.push(south);
  				level[south] = level[now] + 1;
  			}	
  		}
  		cout<<cost<<endl;
  	}
