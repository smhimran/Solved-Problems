...
int main()
{
    ...
    while (t--) {
        ...
        bool ok = 1;
        int ans = 0;
        int x = alpha['A'].first, y = alpha['A'].second;
        visited[x][y]=1;
        grid[x][y]='.';
        for (char c = 'B'; c<='Z' and alpha.count(c); c++) {
            int destx = alpha[c].first, desty = alpha[c].second;
            queue<PII> q;
            q.push({x, y});
            ...
            while (!q.empty()) {
                ...
                for (int i=0; i<4; i++) {
                    int a = x+ROW[i];
                    int b = y+COL[i];
                    if ((a>=1 and a<=n) and (b>=1 and b<=n) and (grid[a][b]=='.' or grid[a][b]==c) and !visited[a][b]) {
                        q.push({a, b});
                        cost[a][b] = cost[x][y]+1;
                        visited[a][b]=1;
                    }
