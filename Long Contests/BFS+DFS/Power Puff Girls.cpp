...
int main()
{
    ...
    while (t--) {
        ...
        // cout<<"-->home "<<home.first<<" "<<home.second<<endl;
        queue<PII> q;
        ...
        while (!q.empty()) {
            ...
            if (now==home)
                break;
            for (int i=0; i<4; i++) {
                int x = now.first+ROW[i];
                int y = now.second+COL[i];
                PII next = {x, y};
                if (grid[x][y]!='m' and grid[x][y]!='#' and !visited[next]) {
        ...
        while (!q.empty()) {
            ...
            for (int i=0; i<4; i++) {
                int x = now.first+ROW[i];
                int y = now.second+COL[i];
                if (grid[x][y]!='m' and grid[x][y]!='#' and !visited[{x, y}]) {
                    dist[{x, y}] = dist[now] + 1;
                    visited[{x, y}] = 1;
                    q.push({x, y});
                }
