...
void multiply(LL F[2][2], LL M[2][2]) {
    LL x, y, z, w;
    x = (F[0][0]%mod * M[0][0]%mod)%mod + (F[0][1]%mod * M[1][0]%mod)%mod;
    y = (F[0][0]%mod * M[0][1]%mod)%mod + (F[0][1]%mod * M[1][1]%mod)%mod;
    z = (F[1][0]%mod * M[0][0]%mod)%mod + (F[1][1]%mod * M[1][0]%mod)%mod;
    w = (F[1][0]%mod * M[0][1]%mod)%mod + (F[1][1]%mod * M[1][1]%mod)%mod;
    F[0][0]=x%mod;
    F[0][1]=y%mod;
    F[1][0]=z%mod;
    F[1][1]=w%mod;
    ...
    // FastIO;
     ...
     freopen("out.txt", "w", stdout);
  	...
  	while (t--) {
  		LL a, b;
  		cin>>a>>b;
  		LL x = fib(a+1) - 1, y = fib(b+2) - 1;
  		cout<<(y%mod - x%mod + mod)%mod<<endl;
  	}
