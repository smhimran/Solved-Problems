...
int main()
{
    // FastIO;
     ...
     freopen("out.txt", "w", stdout);
  	...
  	double a, b, c;
  	while (cin>>a>>b>>c) {
  		double semiperimiter = a+b+c;
  		semiperimiter /= 2.0;
  		double area = (semiperimiter - a) * (semiperimiter - b) * (semiperimiter - c);
  		area *= semiperimiter;
  		area = sqrt(area);
  		double sunflower = a  * b * c;
  		sunflower /= sqrt((a+b+c) * (a+b-c) * (a+c-b) * (b+c-a));
  		sunflower = PI * sunflower * sunflower;
  		sunflower -= area;
  		double roses = area / semiperimiter;
  		roses = PI * roses * roses;
  		double violete = area - roses;
  		printf("%.4lf %.4lf %.4lf\n", sunflower, violete, roses);
  	}
