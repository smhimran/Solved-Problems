    ...
    // FastIO;
     ...
     freopen("out.txt", "w", stdout);
  	...
  	while (t--) {
  		...
  		string s;
  		cin>>s;
  		int one_count = 0, zero_count = 0, q_count = 0, i;
  		bool ok = 1;
  		for (i=0; i<k; i++) {
  			if (s[i] == '1')
  				one_count++;
  		...
  		// debug(one_count, zero_count, q_count);
  		if (q_count) {
  			if (abs(one_count - zero_count) == q_count) {
  				char c = '0';
  			...
  			else if (abs(one_count - zero_count) < q_count) {
  				for (int j=0; j<k; j++) {
  					if (s[j] == '?') {
  						if ((j+k) < n) {
  							char c = s[j+k];
  		...
  		for (i=1; i< n-k +1; i++) {
  			...
  			prev = s[i];
  			// debug(prev, i, one_count, zero_count);
  		}
  		cout<<(ok? "YES":"NO")<<endl;
  	}
