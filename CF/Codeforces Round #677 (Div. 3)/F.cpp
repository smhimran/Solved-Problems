...
int n, m, k, lim, a[75][75], dp[75][75][40][70];

int solve(int i, int j, int taken, int mod) {
	if (i==n) {
		if (mod) 
			return -inf;
		return 0;
	...
	int &ret = dp[i][j][taken][mod];
	if (ret != -1)
		return ret;
	ret = -inf;
	if (taken < lim) 
		ret = max(ret, a[i][j]+ solve(i, j+1, taken+1, (mod+a[i][j])%k));
	ret = max(ret, solve(i, j+1, taken, mod));
	return ret;
}
