...
int main()
{
    ...
    for (int ind=1; ind<=n; ind++) {
        ...
        mx = a[ind];
        if (x<mx) {
            left = (a[ind]*(a[ind]+1))/2 - ((a[ind]-x)*(a[ind]-x+1))/2;
        }
        else if (x==mx) {
            left = (a[ind]*(a[ind]+1))/2;
        }
        else {
            left = (a[ind]*(a[ind]+1))/2;
            right = left;
            LL k = x-a[ind], pos=ind-1;
            while (k) {
                if (pos<1) 
                    pos=n;
                if (k>=a[pos]) {
                    left += (a[pos]*(a[pos]+1))/2;
                    k -= a[pos];
                }
                else {
                    left += (a[pos]*(a[pos]+1))/2 - ((a[pos]-k)*(a[pos]-k+1))/2;
                    k = 0;
                }
                pos--;
            }
