...
int main()
{
    ...
    while (t--) {
    	...
    	int two = -1, five = -1, seven = -1, zero = -1, zero_earlier = -1;
    	...
    	int forZero = inf, forFive = inf;

    	if (zero > 0) {
	    	for (int i=zero - 1; i>=0; i--) {
	    		if (s[i] == '5') {
	    			forZero = (zero - i) - 1;
	    			forZero += len(s) - (zero + 1);
	    	...
	    	if (zero_earlier > 0) {
	    		zeroZero = (zero - zero_earlier) - 1;
	    		zeroZero += len(s) - (zero + 1);
	    	}

	    	forZero = min(forZero, zeroZero);
    	}

    	if (five > 0) {
    		int twoFive = inf, sevenFive = inf;

    		for (int i=five - 1; i>=0; i--) {
	    		if (s[i] == '2') {
	    			twoFive = (five - i) - 1;
	    			twoFive += len(s) - (five + 1);
    		...
    		for (int i=five - 1; i>=0; i--) {
	    		if (s[i] == '7') {
	    			sevenFive = (five - i) - 1;
	    			sevenFive += len(s) - (five + 1);

	    			break;
	    		}
	    	}

	    	forFive = min(twoFive, sevenFive);
    	}

    	// debug(forFive, forZero);

    	cout<<min(forZero, forFive)<<endl;
    }
