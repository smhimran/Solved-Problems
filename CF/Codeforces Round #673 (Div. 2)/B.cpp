    ...
    // FastIO;
     ...
     freopen("out.txt", "w", stdout);
  	...
  	while (t--) {
  		...
  		if ((v[n-1].first + v[n-2].first) < T) {
  			for (int i=0; i<n-1; i++)
  				cout<<0<<" ";
  			cout<<1<<endl;
  			continue;
  		}
  		LL mn = LL(1e18+18), min_index=0, max_index=0;
  		for (int i=1; i<n; i++) {
  			LL sum = v[i-1].first + v[i].first;
  			LL diff = abs(sum - T);
  			if (diff < mn) {
  				mn = diff;
  				min_index = i-1;
  		...
  		// debug(min_index, max_index);
  		for (int i=0; i<=min_index; i++)
  			ans[i]=0;
  		for (int i=max_index; i<n; i++)
  			ans[i]=1;
  		if ((max_index - min_index) > 1) {
  			int half = (max_index+min_index)>>1;
  			// debug(half);
  			for (int i=min_index+1; i<=half; i++)
  				ans[i]=0;
  			for (int i=half+1; i<max_index; i++)
  				ans[i]=1;
  		}
  		for (int i=0; i<n; i++)
  			cout<<ans[i]<<" ";
