...
LL checkpoint[int(1e5+5)], n, l;

long double leftTime(double dis) {
	long double t = 0, v = 1;
	int last = 0;
	for (int i=1; i<=n and checkpoint[i]<dis; i++) {
		long double s = checkpoint[i] - checkpoint[i-1];
		t += s / v;
		v += 1.0;
		last = checkpoint[i];
	}
	long double s = dis - (long double)last;
	t += s / v;
	return t;
}

long double rightTime(double dis) {
	long double t = 0, v = 1;
	int last = l;
	for (int i=n; i>=0 and checkpoint[i]>dis; i--) {
		long double s = checkpoint[i+1] - checkpoint[i];
		t += s / v;
		v += 1.0;
		last = checkpoint[i];
	}
	long double s = (long double)last - dis;
	t += s / v;
	return t;
}
     ...
     freopen("out.txt", "w", stdout);
  	...
  	while (t--) {
  		...
  		for (int i=1; i<=n; i++)
  			cin>>checkpoint[i];
  		checkpoint[n+1] = l;
  		long double left = 0, right = l, mid, lt, rt, time=0;
  		LL k = 100;
  		while (k--) {
  			mid = (left+right)/2.0;
  			lt = leftTime(mid);
